...
### Live result, 2026-09-22

The workflow now exits 0 against real credentials: run [`35758685542`](https://github.com/Fahad9101/NRE/actions/runs/35758685542) at commit `76c3a9f0cfaf3a692f615a72b5f72284aa857e2c` reports `access_check_passed:true`, all 82 expected IBM daily sessions received with zero missing and zero zero-volume rows, SIP/raw/asof/ascending order confirmed, and complete pagination through an observed terminal `next_page_token` across two pages. Full settings, hashes and counts are preserved in `reports/alpaca-github-actions-diagnostic.json`; no raw bars are included. `accepted_for_labels` remains `false` and `state` remains `STAGED_REVIEW_REQUIRED` — this is still a single symbol outside the frozen cohort, and the five listed blockers (session-volume semantics, corporate-action coverage, historical identity, provider rights, durable raw-response archival) remain open research gates, not access problems.

Reaching this state required three fixes, in order: the repository secrets existed under an `ALPACA_` name prefix rather than the `APCA_` prefix the workflow (and Alpaca's own convention) require; once renamed, one of the two values contained a character illegal in an HTTP header value (surfaced as `UNCATEGORIZED_VALUEERROR` at `http.client.putheader`, diagnosed by the safe raise-site capture added to `nre/alpaca_probe.py`); and the first corrected pair was itself rejected by Alpaca with `ALPACA_HTTP_401` before being replaced with a verified-working pair. None of these were code defects in this repository.
