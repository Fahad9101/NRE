...
The real run (`35827234602`) confirmed everything the evidence packet predicted: all 21 sessions present, `release_timing: after_hours` exactly as expected -- and Alpaca's real corporate-actions endpoint found exactly 1 action in the window, almost certainly the announced dividend increase. The event correctly quarantined on `CORPORATE_ACTION_AUDIT_MISSING` rather than fabricating a clean label. This is the pipeline working as designed, not a failure: real evidence of a corporate action was found and respected. Full result in `reports/m1-cxt-acquisition-result-2026-09-23.json`.

## CXT resolved: second complete event — September 23

Fetched the action's actual details rather than just its count: a $0.18 cash dividend, ex-date `2026-02-27`. Direct calendar computation (not hand-counted) confirmed this falls inside `session_20_close_return`'s window (ends 2026-03-12) but not Day-1 through `session_10_close_return` (ends 2026-02-26). `nre/cxt_acquire.py` was rewritten to feed the real ex-date into `bundle["corporate_actions"]` so `nre.dataset`'s own `CORPORATE_ACTION_IN_WINDOW` check does the precise, per-label suppression -- `corporate_actions_verified` is now genuinely earned (the action was actually reviewed) rather than a blanket flag guessing at relevance.

Re-run (`e79b5c7`, run `35828560404`) confirmed it works exactly as intended: `state: MAPPED`, `mapped_day1: true`. CXT opened +1.0%, ran to +8.0% intraday, pulled back to a low of -2.0%, closed +3.4%; drifted to +2.0% (session 2) and +1.8% (session 5) before giving it back to -4.4% by session 10. `session_20_close_return` is honestly left `null` with reason `CORPORATE_ACTION_IN_WINDOW` -- not a gap to fill in later, a correct and permanent result for this event. Full record in `reports/m1-cxt-complete-event-2026-09-23.json`. The ledger now reads `disposition: included` for `0000025445-26-000005`, and `reports/m1-acceptance-review.json` has its second `spot_checks` entry.

**Worth noting precisely**: this does not advance `independent_timing_spot_checks` toward passing. SLSN is `premarket` timing; CXT is `after_hours` -- different classes, and the gate needs 2 distinct events *within* each class, not 2 events total. Each class still needs its own second example.

Current totals: 243 candidates / 215 issuers / 14 excluded / **2 included** / 227 quarantined / **2 complete events across 2 issuers**. Milestone 1 remains BLOCKED.
